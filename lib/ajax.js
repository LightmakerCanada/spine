...
(function() {
  ...
  Base = (function() {
    ...
    Base.prototype.ajax = function(params, defaults, retry) {
      var jqXHR;
      jqXHR = $.ajax(this.ajaxSettings(params, defaults));
      if (retry && typeof jqXHR.retry === 'function') {
        jqXHR = jqXHR.retry(retry);
      }
      return jqXHR;
    };

    Base.prototype.ajaxQueue = function(params, defaults, record, retry) {
      ...
      promise.abort = (function(_this) {
        return function(statusText) {
          var index;
          if (jqXHR) {
            return jqXHR.abort(statusText);
          }
          index = $.inArray(request, _this.queue());
          if (index > -1) {
            _this.queue().splice(index, 1);
          }
          deferred.rejectWith(settings.context || settings, [promise, statusText, '', settings]);
          return promise;
        };
      })(this);
      this.queue(request);
  ...
  Collection = (function(superClass) {
    ...
    Collection.prototype.find = function(id, params, options) {
      ...
      retry = options.retry || this.model.ajaxRetry;
      return this.ajaxQueue(params, {
        type: options.method || Ajax.config.loadMethod,
        url: options.url || Ajax.getURL(record),
        parallel: options.parallel
      }, null, retry).done(this.recordsResponse(options)).fail(this.failResponse(options));
    };
  ...
  Singleton = (function(superClass) {
    ...
    Singleton.prototype.reload = function(params, options) {
      ...
      return this.ajaxQueue(params, {
        ...
        url: options.url,
        parallel: options.parallel
      }, this.record, retry).done(this.recordResponse(options)).fail(this.failResponse(options));
    };
    ...
    Singleton.prototype.create = function(params, options) {
      ...
      return this.ajaxQueue(params, {
        ...
        parallel: options.parallel
      }, null, retry).done(this.recordResponse(options)).fail(this.failResponse(options));
    };
