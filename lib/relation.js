...
(function() {
  ...
  Collection = (function(superClass) {
    ...
    Collection.prototype.fetch = function(params, options) {
      ...
      return typeof (base = this.model).ajax === "function" ? base.ajax().fetch(params, options) : void 0;
    };

    Collection.prototype.url = function() {
      var scope;
      scope = new this.model();
      scope[this.fkey] = this.record.id;
      return Spine.Ajax.getCollectionURL(scope);
    };
    ...
    Collection.prototype.refresh = function(values) {
      var i, j, k, l, len, len1, len2, match, record, ref, ref1, ref2, refreshedIds;
      if (values == null) {
        return this;
      }
      ...
      refreshedIds = values.map(function(value) {
        return value.id;
      });
      ref = this.all();
      for (j = 0, len = ref.length; j < len; j++) {
        record = ref[j];
        if (!(ref1 = record.id, indexOf.call(refreshedIds, ref1) < 0)) {
          continue;
        }
        delete this.model.irecords[record.id];
        ref2 = this.model.records;
